<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.dakatu" label="打卡图" prop="dakatu">
          <file-upload
          tip="点击上传打卡图"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.dakatu?ruleForm.dakatu:''"
          @change="dakatuUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.dakatu" label="打卡图" prop="dakatu">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.dakatu.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.kaoqinleixingOptions = "签到,签退".split(',')
            this.kaoqinzhuangtaiOptions = "正常,迟到,早退,旷工,请假".split(',')
    },
    // 下二随
    ...
    dakatuUploadChange(fileUrls) {
	this.ruleForm.dakatu = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
