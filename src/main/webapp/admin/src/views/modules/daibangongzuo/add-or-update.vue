<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="流程代码" prop="liuchengdaima">
          <el-input v-model="ruleForm.liuchengdaima" 
              placeholder="流程代码" clearable  :readonly="ro.liuchengdaima"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="流程代码" prop="liuchengdaima">
              <el-input v-model="ruleForm.liuchengdaima" 
                placeholder="流程代码" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="流程类型" prop="liuchengleixing">
          <el-select v-model="ruleForm.liuchengleixing" placeholder="请选择流程类型">
            <el-option
                v-for="(item,index) in liuchengleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="流程类型" prop="liuchengleixing">
	      <el-input v-model="ruleForm.liuchengleixing"
                placeholder="流程类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="提交表格附件" prop="tijiaobiaogefujian">
          <file-upload
          tip="点击上传提交表格附件"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.tijiaobiaogefujian?ruleForm.tijiaobiaogefujian:''"
          @change="tijiaobiaogefujianUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.tijiaobiaogefujian" label="提交表格附件" prop="tijiaobiaogefujian">
            <el-button type="text" size="small" @click="download(ruleForm.tijiaobiaogefujian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="审批回复" prop="shenpihuifu">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="审批回复"
                  v-model="ruleForm.shenpihuifu" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shenpihuifu" label="审批回复" prop="shenpihuifu">
                    <span>{{ruleForm.shenpihuifu}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
    ...
    return {
      ...
      ro:{
	liuchengdaima : false,
	liuchengleixing : false,
	tijiaobiaogefujian : false,
	...
	shenpihuifu : false,
      ...
      rules: {
          liuchengdaima: [
                { required: true, message: '流程代码不能为空', trigger: 'blur' },
          ],
          liuchengleixing: [
          ],
          tijiaobiaogefujian: [
  ...
  created() {
	...
	this.ruleForm.shenpishijian = this.getCurDateTime()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='liuchengdaima'){
            this.ruleForm.liuchengdaima = obj[o];
	    this.ro.liuchengdaima = true;
            continue;
          }
          if(o=='liuchengleixing'){
            this.ruleForm.liuchengleixing = obj[o];
	    this.ro.liuchengleixing = true;
            continue;
          }
          if(o=='tijiaobiaogefujian'){
            this.ruleForm.tijiaobiaogefujian = obj[o];
	    this.ro.tijiaobiaogefujian = true;
            continue;
          }
	    ...
	    this.ro.shenpishijian = true;
          ...
          if(o=='shenpihuifu'){
            this.ruleForm.shenpihuifu = obj[o];
	    this.ro.shenpihuifu = true;
            continue;
          }
        }
      }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `daibangongzuo/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.daibangongzuoCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    tijiaobiaogefujianUploadChange(fileUrls) {
	this.ruleForm.tijiaobiaogefujian = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
