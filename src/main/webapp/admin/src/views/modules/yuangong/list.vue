<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('yuangong','生成档案') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="renshidanganCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'生成档案':'' }}</el-button>
                <el-button v-if="isAuth('yuangong','生成档案') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="renshidanganCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'生成档案':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yuangong','生成档案') && contents.tableBtnIcon == 0" type="success" size="mini" @click="renshidanganCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'生成档案':'' }}</el-button>
    ...
    <renshidangan-cross-add-or-update v-if="renshidanganCrossAddOrUpdateFlag" :parent="this" ref="renshidanganCrossaddOrUpdate"></renshidangan-cross-add-or-update>
...
import renshidanganCrossAddOrUpdate from "../renshidangan/add-or-update";
export default {
  data() {
    return {
      bumenOptions: [],
      ...
      renshidanganCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    renshidanganCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.renshidanganCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','yuangong');
      this.$nextTick(() => {
      this.$refs.renshidanganCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
          this.$http({
          ...
          this.$http({
            url: `option/gangweixinxi/gangwei`,
            method: "get"
          }).then(({ data }) => {
            if (data && data.code === 0) {
