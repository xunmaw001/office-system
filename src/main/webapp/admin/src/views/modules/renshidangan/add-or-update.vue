<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="基本薪资" prop="jibenxinzi">
          <el-input v-model="ruleForm.jibenxinzi" 
              placeholder="基本薪资" clearable  :readonly="ro.jibenxinzi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="基本薪资" prop="jibenxinzi">
              <el-input v-model="ruleForm.jibenxinzi" 
                placeholder="基本薪资" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="工作履历" prop="gongzuolvli">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.gongzuolvli" label="工作履历" prop="gongzuolvli">
                    <span>{{ruleForm.gongzuolvli}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.gongzuolvli = true;
          ...
          if(o=='jibenxinzi'){
            this.ruleForm.jibenxinzi = obj[o];
	    this.ro.jibenxinzi = true;
            continue;
          }
          if(o=='gengxinshijian'){
      ...
      this.$http({
		...
		if(json.zhaopian!=''&&json.zhaopian){
          ...
          this.$message.error(data.msg);
            ...
            this.zaizhizhuangtaiOptions = "在职,已离职".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `renshidangan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `renshidangan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.renshidanganCrossAddOrUpdateFlag = false;
                  this.parent.search();
